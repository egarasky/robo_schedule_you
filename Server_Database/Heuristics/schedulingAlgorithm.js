...
theScheduler.schedule = function(callback){
	...
	theScheduler.shifts.forEach(function(shift){
		...
		passedAll.sort(theScheduler.weights[theScheduler.weights.length - 1]);
		passedAll.forEach(function(employee){
...
theScheduler.collapse = function(layer, layersOfRejection, callback){
	theScheduler.shifts.forEach(function(shift){
		var empsOfLayer = layersOfRection[shift_.id][layer];
	...
	if(layer === 0) return;
	layer -= 1;
	setTimeout(Scheduler.collapse.bind(null, layer, layersOfRejection, callback));
};
