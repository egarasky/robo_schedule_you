...
BusinessController.roles = function(req, res){
	...
	//use session cookie to get userId and look up user roles
	var userId = req.session._id;
	User.findOne({_id:userId.toObjectId()}, function(err, user){
		if(err) throw err;
		if(!user) throw err;//user with _id from cookie does not exist
		res.json(user.roles);

	});
};
BusinessController.addRole = function(req, res){
...
BusinessController.deleteRole = function(req, res){
